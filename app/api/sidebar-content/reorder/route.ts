...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const { id, direction, position } = body;
    const dir = direction as 'up' | 'down';

    const rows = await query<{ id: string; sort_order: number }>(
      'SELECT id, sort_order FROM sidebar_content WHERE position = $1 ORDER BY sort_order',
      [position]
    );
    ...
    await query('UPDATE sidebar_content SET sort_order = $1 WHERE id = $2', [
      swap.sort_order,
      current.id,
    ]);
    await query('UPDATE sidebar_content SET sort_order = $1 WHERE id = $2', [
